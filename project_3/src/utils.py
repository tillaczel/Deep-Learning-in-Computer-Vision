...
class ImagePool:
    ...
    def __init__(self, pool_sz: int = 300):
    ...
    def push_and_pop(self, images, device):
        """
        Parameters:
        ...
        for image in images:
            ...
            if self.nb_images < self.pool_sz:
                self.image_pool.append(image.detach().cpu())
                ...
                if np.random.uniform(0, 1) > 0.6:
                    ...
                    self.image_pool[rand_int] = image.detach().cpu()
                    images_to_return.append(temp_img.to(device))
                else:
                    images_to_return.append(image)
