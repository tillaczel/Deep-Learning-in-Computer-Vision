...
class IoU(Metric):
    ...
    def update(self, preds: torch.Tensor, target: torch.Tensor):
        ...
        intersection = torch.sum(preds_binary == target, dim=self.spatial_dim)
        union = torch.sum(preds_binary | target, dim=self.spatial_dim)
    ...
    def compute(self):
        if self.average == 'macro':
            return torch.mean(self.iou_sum.float(), dim=self.label_dim) / self.total
        else:
